...
// INICIALIZACAO
void iniciarAlegro();
void loadTextures(ALLEGRO_BITMAP *texture[10]);
...
void draw_map(Object **map, int row, int col, Miner m, ALLEGRO_BITMAP *texture[10]);
...
void state_init(){
    ...
    row = level[9].row;
    col = level[9].col; /* map size taken as the size of the biggest map */

    map = (Object **)malloc(row * sizeof(Object *));
    map[0] = (Object *)malloc(row * col * sizeof(Object));
    for (int i = 0; i < row; i++)
        map[i] = map[0] + i * col;
...
void init_map(Object **map, ALLEGRO_BITMAP *texture[10], int r, int c, char *file_name){
    ...
    for (int i = 0; i < r; i++){
        for (int j = 0; j < c; j++){
            switch (s = fgetc(fp)){
            case 'M':
                map[i][j].ObjectID = MINER;
                //  map[i][j].image = texture[MINER];
                break;
            case ' ':
                map[i][j].ObjectID = EMPTY;
                // map[i][j].image = texture[EMPTY];
                break;
            case 'E':
                map[i][j].ObjectID = EARTH;
                // map[i][j].image = texture[EARTH];
                break;
            case 'B':
                map[i][j].ObjectID = BORDER;
                // map[i][j].image = texture[BORDER];
                break;
            case 'R':
                map[i][j].ObjectID = ROCK;
                // map[i][j].image = texture[ROCK];
                break;
            case '*':
                map[i][j].ObjectID = DIAMOND;
                // map[i][j].image = texture[DIAMOND];
                break;
            case 'S':
                map[i][j].ObjectID = SPIDER;
                // map[i][j].image = texture[SPIDER];
                break;
            case '!':
                map[i][j].ObjectID = MONSTER;
                // map[i][j].image = texture[MONSTER];
                break;
            case 'W':
                map[i][j].ObjectID = WATER;
                // map[i][j].image = texture[WATER];
                break;
            case 'D':
                map[i][j].ObjectID = DOOR;
                // map[i][j].image = texture[DOOR];
                break;
            default:
                j--;
            }
...
void state_play(){
    while (!doexit){
        ...
        if (game_state == PLAY && ev.timer.source == miner_timer)
        {
            redraw = true;
            if (keys[UP])
                update_miner(map, texture, &miner, rock, diamond, r_num, d_num, UP, &result);
            else if (keys[DOWN])
                update_miner(map, texture, &miner, rock, diamond, r_num, d_num, DOWN, &result);
            else if (keys[RIGHT])
                update_miner(map, texture, &miner, rock, diamond, r_num, d_num, RIGHT, &result);
            else if (keys[LEFT])
                update_miner(map, texture, &miner, rock, diamond, r_num, d_num, LEFT, &result);
        ...
        if (game_state == PLAY && redraw && al_is_event_queue_empty(event_queue)){
            redraw = false;
            check_dead(&miner);
            draw_map(map, row, col, miner, texture);
...
// FUNCOES PARA ATUALIZAR OBJETOS
    ...
    if (move == UP)
    {
        ...
        case DOOR:
            *result = 1;
            return;
        case EMPTY:
            al_play_sample_instance(walk_empty);
            break;
        case EARTH:
            al_play_sample_instance(walk_earth);
            break;
        case DIAMOND:
...
void camera_update(Position *camera_position, int x, int y, int width, int height, int row, int col, Level lvl)
{
    ...
    if (x > (float)col / lvl.x_radius)
        x = (float)col / lvl.x_radius + 0.5;
    if (y > (float)row / lvl.y_radius)
        y = (float)row / lvl.y_radius + 0.5;
...
void draw_map(Object **map, int row, int col, Miner m, ALLEGRO_BITMAP *texture[10]){
    ...
    for (int y = 0; y < row; y++){
        for (int x = 0; x < col; x++){
            ...
            // {
            // default:
                al_draw_bitmap(texture[map[y][x].ObjectID], x * SIZE, y * SIZE, 0);
